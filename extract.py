...
def filter_company_info(ticker_id):
    ...
    if ticker_id not in full_info_json or not isinstance(full_info_json[ticker_id], dict):
        print(f"Warning: No valid data returned from API for ticker {ticker_id}")
        return {}

    data = full_info_json[ticker_id]
    # ---------------------------------------------------------
...
def about(asset_profile, quote_type, price):
    ...
    return {
        ...
        "Full Time employees": asset_profile.get("fullTimeEmployees", 0),
...
def finance(price,tot_rev,summary):
    ...
    return {
        ...
        "Current Price": price.get("regularMarketPrice", 0),
        "Market Cap": price.get("marketCap", 0),
        "Total Revenue": tot_rev.get("totalRevenue", 0),
        "Revenue Growth": tot_rev.get("revenueGrowth", 0),
        "Profit Growth": tot_rev.get("earningsGrowth", 0),
