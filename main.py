    ...
    try:
        ...
        with pd.ExcelWriter(excel_file_path, engine='xlsxwriter') as writer:
            ...
            primary_header_format = workbook.add_format({
                ...
                'align': 'center',
                'valign': 'vcenter',
                # 'fg_color': '#44546A',  # Dark Blue-Gray
                'fg_color': '#0b5394',
                'border': 1
            })
            ...
            section_data = []
            section_order = ["About", "Finance", "Location", "Growth", "Buyer Group", "Mutual Fund Holders", "Ownership Structure"]
            ...
            for section_title, df in section_data:
                ...
                num_cols = df.shape[1]
                if num_cols > 1:
                    worksheet.merge_range(0, current_col, 0, current_col + num_cols - 1, section_title, primary_header_format)
                else:
                ...
                for r_idx, row in enumerate(df.itertuples(index=False), start=2):
                    for c_idx, value in enumerate(row):
                        ...
                        # Choose the correct format for the cell
                        cell_format = default_cell_format
                        if col_name in ['Current Price', 'Market Cap', 'Total Revenue', 'open', 'high', 'low', 'close', 'adjclose', 'Shares']:
                ...
                for i, col_name in enumerate(df.columns):
                    ...
                    width = max(data_width if not pd.isna(data_width) else 0, len(str(col_name)))
